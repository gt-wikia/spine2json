...
out += `\n`;
out += `:scale0\n`;
out += `set _filters=[0:v]scale=iw:ih:flags=spline[v]\n`;
out += `\n`;
out += `:pad\n`;
out += `set _filters=%_filters%;[v]pad=iw+%_pl%+%_pr%:ih+%_pt%+%_pb%:%_pl%:%_pt%:#00000000[v]\n`;
out += `\n`;
out += `:crop\n`;
out += `set _do_crop=0\n`;
out += `set _offset_t=0\n`;
...
out += `set _filters=%_filters%;[v]crop=iw-%_cl%-%_cr%-%_pl%-%_pr%:ih-%_ct%-%_cb%-%_pt%-%_pb%:%_offset_l%:%_offset_t%[v]\n`;
out += `\n`;
out += `:scale\n`;
out += `set _filters=%_filters%;[v]scale=iw/2:ih/2:flags=spline\n`;
out += `\n`;
out += `:encode\n`;
