...
out += `\n`;
out += `:scale\n`;
out += `set _filters=scale=iw/2:ih/2:flags=spline\n`;
out += `\n`;
out += `:pad\n`;
...
out += `set _offset_t=0\n`;
out += `set _offset_l=0\n`;
out += `if %_ct% == 0 (set _pt=0) else (set crop=1 & set /a _offset_t=%_ct%+%_pt%)\n`;
out += `if %_cl% == 0 (set _pl=0) else (set crop=1 & set /a _offset_l=%_cl%+%_pl%)\n`;
out += `if %_cr% == 0 (set _pr=0) else (set crop=1)\n`;
out += `if %_cb% == 0 (set _pb=0) else (set crop=1)\n`;
out += `if not defined crop goto encode\n`;
...
out += `echo %_filters%\n`;
out += `ffmpeg -hide_banner -framerate 18 -i "./${process.env._frames_dir}/${process.env._input}-idle_%%02d.png" ^\n`;
